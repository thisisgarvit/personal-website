...
| DD-011 | Amplitude informs funnel hierarchy and terminology, not trade dress. The dominant funnel is this visitor’s live browser-local session (reached/not reached); the diminished `typical visitor` pane is a static authored benchmark and must be labeled as such. | Locked semantics; pixels pending | Garvit + Codex | Never present aggregate counts as the current visitor’s data. Validate the visual treatment in the dedicated cockpit mock. |
...
| DD-013 | V1 reuses the existing live Muko/WebGL guide for welcome and meaningful session reactions, with matching poster fallbacks. V2 may use an original vintage-anime 2.5D guide. Do not use literal Zoro or copied IP. | Locked scope | Garvit + Codex | Do not reopen character production in V1. Live 3D is justified only by meaningful continuous reactions. |
...
| DD-018 | Cockpit trust copy remains exact: “this funnel is computed in your browser. PostHog sees the rest. I check it obsessively.” | Locked | Garvit + Codex | Persona goes to PostHog; the session funnel remains browser-local. Do not claim that PostHog receives nothing else. |
